# LINE Daily Brief システム

Obsidian Vault から特定タグ付きのメモを抽出し、LINE公式アカウントから毎日1件配信するシステム。

## 📋 概要

```
┌─────────────────────────────────────────────────────────────────┐
│                        Obsidian Vault                          │
│  /Users/masayuki/Obsidian Pro Kit for market                   │
│                                                                 │
│  - メモに #cv_line タグを付けるだけ                              │
│  - テーマタグ (#ai_gov, #tax 等) で分類可能                     │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         │ deno task export (手動 or 定期実行)
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                      Supabase (line_cards)                      │
│                                                                 │
│  - 抽出されたカードを保存                                        │
│  - 重複チェック (content_hash)                                  │
│  - テーマ・配信回数を管理                                        │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         │ GitHub Actions / Cron (毎日1回)
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│              Edge Function (line-daily-brief)                   │
│                                                                 │
│  - テーマの偏りが少ないカードを1件選択                           │
│  - LINE Messaging API でブロードキャスト                        │
│  - 配信後にレコードを更新                                        │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                  LINE 公式アカウント                            │
│                                                                 │
│  🤖 今日のひとこと                                              │
│                                                                 │
│  [カード本文がここに表示される]                                  │
│                                                                 │
│  ──────────                                                     │
│  Cursorvers｜医療AIガバナンス                                   │
└─────────────────────────────────────────────────────────────────┘
```

---

## 🏷️ Obsidian でやること

### 基本ルール

1. **配信したい行に `#cv_line` タグを付ける**
2. **テーマタグを併記すると分類される（任意）**

### 使用可能なテーマタグ

| タグ | 意味 | 配信時の絵文字 |
|------|------|----------------|
| `#ai_gov` | 医療AIガバナンス | 🤖 |
| `#tax` | 税務・資産形成 | 💰 |
| `#law` | 法務・契約 | ⚖️ |
| `#biz` | Cursorvers事業戦略 | 📈 |
| `#career` | 医師キャリア・働き方 | 👨‍⚕️ |
| `#asset` | 個人の資産形成 | 🏦 |
| （タグなし） | 一般 | 💡 |

### サンプルノート

```markdown
# AIガバナンスのメモ

## 重要なポイント

- 医療AIの導入には、必ず院内ガバナンス委員会の承認を得ること #cv_line #ai_gov
- AI診断支援は「最終判断は医師」が鉄則。責任の所在を契約書に明記 #cv_line #law
- 当直代を減らすより、知識を売る方がコスパが良い #cv_line #career

## 税務メモ

- コンサル収入は雑所得ではなく事業所得として申告できる場合がある #cv_line #tax
```

### 注意事項

- **PHI（患者情報）は絶対にタグ付けしない**
  - このシステムは「タグ付き行」のみを外部に送信します
  - 患者情報や機密情報には `#cv_line` を付けない運用を徹底してください

- **1行が1カードになる**
  - 長すぎる場合は、前後1-2行が含まれることがあります
  - 読みやすい分量（1-3文程度）を心がけてください

---

## 🔧 セットアップ

### 1. 環境変数の設定

```bash
# .env ファイルを作成
cp .env.example .env

# 以下を設定
VAULT_PATH=/Users/masayuki/Obsidian Pro Kit for market
SUPABASE_URL=https://your-project.supabase.co
SUPABASE_SERVICE_ROLE_KEY=your-service-role-key
LINE_CHANNEL_ACCESS_TOKEN=your-line-channel-access-token
CRON_SECRET=your-random-secret-for-scheduler
```

### 2. データベースマイグレーション

```bash
cd cursorvers_line_stripe_discord

# マイグレーションを適用
supabase db push
```

### 3. 同期スクリプトのテスト

```bash
cd scripts/export-line-cards

# ドライラン（データベースには書き込まない）
deno task export:dry

# 本番実行
deno task export
```

### 4. Edge Function のデプロイ

```bash
cd cursorvers_line_stripe_discord

# デプロイ
supabase functions deploy line-daily-brief

# 環境変数を設定
supabase secrets set LINE_CHANNEL_ACCESS_TOKEN=your-token
supabase secrets set CRON_SECRET=your-secret
```

---

## 📅 スケジューラの設定

### GitHub Actions (推奨)

`.github/workflows/line-daily-brief.yml`:

```yaml
name: LINE Daily Brief

on:
  schedule:
    # 毎日 07:00 JST (22:00 UTC)
    - cron: '0 22 * * *'
  workflow_dispatch: # 手動実行も可能

jobs:
  send-brief:
    runs-on: ubuntu-latest
    steps:
      - name: Trigger LINE Daily Brief
        run: |
          curl -X POST \
            -H "X-CRON-SECRET: ${{ secrets.LINE_DAILY_BRIEF_CRON_SECRET }}" \
            -H "Content-Type: application/json" \
            "${{ secrets.SUPABASE_URL }}/functions/v1/line-daily-brief"
```

### Manus から叩く場合

```bash
curl -X POST \
  -H "X-CRON-SECRET: your-cron-secret" \
  -H "Content-Type: application/json" \
  "https://your-project.supabase.co/functions/v1/line-daily-brief"
```

---

## 📊 運用

### カードの状態管理

| status | 意味 |
|--------|------|
| `ready` | 配信可能（新規カード） |
| `used` | 配信済み（再配信される可能性あり） |
| `archived` | アーカイブ済み（配信対象外） |

### カード選択ロジック

1. `status = 'ready'` のカードから選択
2. テーマごとの累計配信回数（`times_used`）を集計
3. **最も配信回数が少ないテーマ**から1件を選択
4. 同じテーマ内では `times_used` が少ないものを優先
5. 直前に配信したテーマは避けてバラつきを出す（候補が2テーマ以上ある場合）

これにより、テーマの偏りなく配信されます。

### 同期の定期実行（オプション）

Obsidian の更新を定期的に Supabase に反映したい場合：

```yaml
# .github/workflows/sync-line-cards.yml
name: Sync LINE Cards

on:
  schedule:
    - cron: '0 0 * * *' # 毎日 09:00 JST
  workflow_dispatch:

jobs:
  sync:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      
      - uses: denoland/setup-deno@v1
        with:
          deno-version: v1.x
          
      - name: Sync cards
        env:
          VAULT_PATH: ${{ secrets.VAULT_PATH }}
          SUPABASE_URL: ${{ secrets.SUPABASE_URL }}
          SUPABASE_SERVICE_ROLE_KEY: ${{ secrets.SUPABASE_SERVICE_ROLE_KEY }}
        run: |
          cd scripts/export-line-cards
          deno task export
```

---

## 🔍 トラブルシューティング

### カードが抽出されない

1. `#cv_line` タグが正しく付いているか確認
2. タグの前後にスペースがあるか確認（`- text #cv_line` ✓）
3. ドライランで確認: `deno task export:dry`

### LINE配信が失敗する

1. `LINE_CHANNEL_ACCESS_TOKEN` が正しいか確認
2. LINE公式アカウントがアクティブか確認
3. Supabase Functions のログを確認

### テーマが `general` になる

テーマタグ (`#ai_gov` 等) が `#cv_line` と同じ行にあるか確認してください。

---

## 📁 ファイル構成

```
cursorvers_line_stripe_discord/
├── supabase/
│   ├── migrations/
│   │   └── 004_line_cards.sql      # テーブル定義
│   └── functions/
│       └── line-daily-brief/
│           ├── index.ts            # Edge Function
│           └── supabase.toml       # 設定
├── scripts/
│   └── export-line-cards/
│       ├── deno.json               # Deno設定
│       ├── src/
│       │   ├── main.ts             # エントリポイント
│       │   ├── extractor.ts        # 抽出ロジック
│       │   ├── supabase-client.ts  # DB操作
│       │   └── types.ts            # 型定義
│       └── test/
│           └── extractor_test.ts   # テスト
└── docs/
    └── LINE_DAILY_BRIEF.md         # このドキュメント
```
