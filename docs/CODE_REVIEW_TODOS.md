...
- [x] 13. RUNBOOK作成
- [x] 14. 監視・アラート設定（GitHub Actions失敗通知は実装済み）
- [x] 15. データベース関数追加（005_line_cards_functions.sqlに実装済み）
- [x] 16. 配信履歴テーブル活用（006_line_card_broadcasts.sql作成、Edge Functionに実装済み）
- [ ] 17. エラーハンドリング統一
- [x] 18. 環境変数ドキュメント化
- [x] 19. 同期スクリプト定期実行（sync-line-cards.yml作成）
- [x] 20. メトリクス記録実装（配信履歴テーブルとビューで実現）
