...
async function fetchExistingValues(token, columns = 'A:H') {
  const range = encodeURIComponent(`${SHEET_NAME}!${columns}`);
...
async function updateRow(token, rowIndex, values) {
  const range = `${SHEET_NAME}!A${rowIndex}:E${rowIndex}`;
...
async function ensureHeader(token) {
  const existing = await fetchExistingValues(token, 'A1:E1');
...
async function main() {
  ...
  const existing = await fetchExistingValues(accessToken, 'A2:H');
