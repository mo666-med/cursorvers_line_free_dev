...
【必ず出力する情報】
- 元のニュースソース（公開情報）を必ず具体的に引用してください
- 「複数の報道」「各種報道」などの曖昧な表現は禁止です
- 具体的な情報源の例:
  - 政府機関: 「CISA（米サイバーセキュリティ庁）」「FBI/IC3」「NISC（内閣サイバーセキュリティセンター）」
  - 脆弱性: 「CVE-2025-XXXX」「Microsoft Security Advisory」「Apache Security Bulletin」
  - ベンダー: 「CrowdStrike Threat Report」「Mandiant Blog」「Palo Alto Unit42」
  - 報道: 具体的な媒体名「BleepingComputer」「SecurityWeek」「ITmedia」など
- 元の情報源が特定できない場合は、トピックの内容から「オンライン会議セキュリティに関する業界警告」のように記載
- 各トピックの元の報告日または公開日（不明な場合はメール受信日を使用）
